...
public class Conversions {
    ...
    /**
     * Gets the appropriate elevator goal based on the scoring location
     * 
     * @param scoreLocation The desired scoring position (LEFT or RIGHT)
     * @return The elevator goal position (3 for LEFT, 2 for RIGHT)
     */
    public static int getElevatorGoal(ScoreLocation scoreLocation) {
        return switch (scoreLocation) {
            case LEFT2 -> 2;  // Distance left from tag
            case LEFT3 -> 3;  // Distance left from tag
            case RIGHT2 -> 2; // Distance right from tag
            case RIGHT3 -> 3; // Distance right from tag
        default -> throw new IllegalArgumentException("Invalid score location: " + scoreLocation);
        };
    }
