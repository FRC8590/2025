  ...
  public DeepClimbAutoClimbUp() {
    addCommands(
      new ParallelCommandGroup(
        ...
        Commands.run(Constants.LEDSystem::setClimb, Constants.LEDSystem)
      ).until(new BooleanSupplier() {
        public boolean getAsBoolean() {
          return Math.abs(Math.toDegrees(Constants.drivebase.swerveDrive.imuReadingCache.getValue().getY())) > 85;
        }
